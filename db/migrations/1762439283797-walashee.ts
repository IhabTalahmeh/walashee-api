...
export class Walashee1762439283797 implements MigrationInterface {
    name = 'Walashee1762439283797'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`users\` ADD \`useAs\` enum ('admin', 'customer', 'agent', 'delivery_company', 'driver', 'supplier', 'store_manager') NOT NULL DEFAULT 'customer'`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`countries\` CHANGE \`deletedAt\` \`deletedAt\` timestamp(6) NULL`);
        await queryRunner.query(`ALTER TABLE \`user_emails\` CHANGE \`deletedAt\` \`deletedAt\` timestamp(6) NULL`);
        await queryRunner.query(`ALTER TABLE \`users\` DROP COLUMN \`useAs\``);
    }

...
export class Walashee1763237846326 implements MigrationInterface {
    name = 'Walashee1763237846326'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`team_invitation_requests\` ADD \`status\` enum ('pending', 'accepted', 'rejected') NOT NULL DEFAULT 'pending'`);
        await queryRunner.query(`ALTER TABLE \`team_invitation_requests\` ADD \`processedAt\` timestamp NULL`);
        await queryRunner.query(`ALTER TABLE \`team_invitation_requests\` ADD \`processedById\` varchar(36) NULL`);
        ...
        await queryRunner.query(`ALTER TABLE \`team_invitation_requests\` ADD CONSTRAINT \`FK_b06fb130acba1d1046734d7e711\` FOREIGN KEY (\`processedById\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`team_invitation_requests\` DROP FOREIGN KEY \`FK_b06fb130acba1d1046734d7e711\``);
        await queryRunner.query(`ALTER TABLE \`fcm_tokens\` CHANGE \`deletedAt\` \`deletedAt\` timestamp(6) NULL`);
        await queryRunner.query(`ALTER TABLE \`verification_codes\` CHANGE \`deletedAt\` \`deletedAt\` timestamp(6) NULL`);
        await queryRunner.query(`ALTER TABLE \`users\` CHANGE \`deletedAt\` \`deletedAt\` timestamp(6) NULL`);
        await queryRunner.query(`ALTER TABLE \`password_reset_codes\` CHANGE \`deletedAt\` \`deletedAt\` timestamp(6) NULL`);
        ...
        await queryRunner.query(`ALTER TABLE \`countries\` CHANGE \`deletedAt\` \`deletedAt\` timestamp(6) NULL`);
        await queryRunner.query(`ALTER TABLE \`user_emails\` CHANGE \`deletedAt\` \`deletedAt\` timestamp(6) NULL`);
        await queryRunner.query(`ALTER TABLE \`team_invitation_requests\` DROP COLUMN \`processedById\``);
        await queryRunner.query(`ALTER TABLE \`team_invitation_requests\` DROP COLUMN \`processedAt\``);
        await queryRunner.query(`ALTER TABLE \`team_invitation_requests\` DROP COLUMN \`status\``);
    }
